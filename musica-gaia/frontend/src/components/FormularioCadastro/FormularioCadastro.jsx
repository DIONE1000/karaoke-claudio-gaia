...
export function FormularioCadastro({
  ...
  return (
    <>
      <h2 className="text-xs font-bold text-amber-500 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
        <Music4 size={18} className="text-cyan-400" /> Cadastrar Música
      </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-4 space-y-1 relative">
            ...
            <div className="relative">
              <input
                ...
                required
                className="w-full bg-slate-950 border border-indigo-500/20 rounded-xl px-4 py-3 focus:border-amber-500 outline-none transition-all placeholder:text-slate-700 text-slate-100"
            ...
            {showSugestoesCantor && sugestoesCantor.length > 0 && (
              <div ref={sugestoesCantorRef} className="absolute z-20 w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-48 overflow-y-auto">
                {sugestoesCantor.map((nome, i) => (
                  <button key={i} type="button"
                    className="w-full text-left px-4 py-3 hover:bg-amber-500/20 transition-colors text-sm border-b border-indigo-500/10 last:border-0 flex items-center gap-2"
                    onClick={() => selecionarSugestaoCantor(nome)}>
                    <Mic2 size={14} className="text-amber-500 shrink-0" />
                    <span className="text-slate-200">{nome}</span>
                  </button>
                ))}
              </div>
          ...
          <div className="lg:col-span-2 space-y-1 relative">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 focus:border-amber-500 outline-none transition-all placeholder:text-slate-700"
            ...
            {showSugestoesLocal && sugestoesLocal.length > 0 && (
              <div ref={sugestoesLocalRef} className="absolute z-20 w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-40 overflow-y-auto">
                {sugestoesLocal.map((nome, i) => (
                  <button key={i} type="button"
                    className="w-full text-left px-3 py-2.5 hover:bg-slate-400/10 transition-colors text-xs border-b border-indigo-500/10 last:border-0 font-mono text-amber-500"
            ...
            {showSugestoesVersao && sugestoesVersao.length > 0 && (
              <div ref={sugestoesVersaoRef} className="absolute z-20 w-full mt-1 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-40 overflow-y-auto">
                {sugestoesVersao.map((nome, i) => (
                  <button key={i} type="button"
                    className="w-full text-left px-3 py-2.5 hover:bg-slate-400/10 transition-colors text-xs border-b border-indigo-500/10 last:border-0 text-amber-500"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 mt-6 items-end">
          ...
          <div className="lg:col-span-9 space-y-3 border border-slate-800 rounded-xl p-4 bg-slate-950/50">
            <div className="flex items-center justify-between">
              ...
              <button type="button"
                ...
                className={`relative w-11 h-6 rounded-full transition-colors duration-200 ${formData.linkAtivo ? "bg-amber-500" : "bg-slate-700"}`}
              >
            ...
            {formData.linkAtivo && (
              <div className="grid grid-cols-1 md:grid-cols-[1fr,auto] gap-4">
                <input type="url" placeholder="Cole o link aqui..."
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 focus:border-slate-400 outline-none transition-all placeholder:text-slate-700 text-sm"
                ...
                <div className="flex gap-2 p-1 bg-slate-950 rounded-lg min-w-[200px]">
                  <button type="button"
                    ...
                    className={`flex-1 py-2 px-3 text-xs font-bold rounded-md transition-all ${
                      formData.linkTipo === "cantor" ? "bg-slate-400 text-slate-950 shadow-lg shadow-slate-900/40" : "text-slate-500 hover:text-slate-300 hover:bg-slate-900"
                    }`}>Sobre o Cantor</button>
                  <button type="button"
                    onClick={() => setFormData({ ...formData, linkTipo: "musica" })}
                    className={`flex-1 py-2 px-3 text-xs font-bold rounded-md transition-all ${
                      formData.linkTipo === "musica" ? "bg-slate-400 text-slate-950 shadow-lg shadow-slate-900/40" : "text-slate-500 hover:text-slate-300 hover:bg-slate-900"
          ...
          <div className="lg:col-span-3">
            <button type="submit"
              ...
              className={`w-full ${isProcessing ? 'bg-slate-700 cursor-not-allowed' : 'bg-amber-500 hover:bg-amber-600 active:bg-amber-700'} text-slate-950 font-black rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-amber-900/40 active:scale-95 ${formData.linkAtivo ? 'h-full py-3' : 'py-4'}`}
